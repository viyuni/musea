...
describe('artManifestVirtualFile', () => {
  ...
  test('propagates manifest timestamp to art module loaders', async () => {
    const artManifest: ArtManifest[] = [
      {
        id: 'src/Button.art.vue',
        file: 'src/Button.art.vue',
    ...
    const code = await artManifestVirtualFile.load(
      createContext(artManifest, new URLSearchParams({ t: '123456' })),
    );

    expect(code).toContain(
      '"src/Button.art.vue": () => import("virtual:musea-art?artId=src%2FButton.art.vue&t=123456")',
    );
  });
